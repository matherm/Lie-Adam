...
class Net2d(nn.Module):
    ...
    def __init__(self, inpt_shape, out_channels, filter_size, stride, ds_size, init_eye=False):
        super().__init__()
...
class ConvSpatialICA(SpatialICA):
    ...
    def fit(self, X, epochs, X_val, lr=1e-3, *args, **kwargs):
        ...
        if self.net is None:
            ...
            self.net = Net2d(self.shape, self.n_components, filter_size=self.BSZ[0], stride=self.stride,  ds_size=int(len(X) * self.optimistic_whitening_rate), init_eye=self.init_eye)
            self.reset(lr)
