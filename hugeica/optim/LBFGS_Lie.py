...
class LBFGS_Lie(torch.optim.Optimizer):
    ...
    def _gather_flat_grad(self):
        ...
        for p in self._params:
            ...
            elif isinstance(p, SOParameter):
                triuidx = torch.triu_indices(*p.grad.shape, offset=1).to(p.grad.device)
                view = p.grad.data[triuidx[0], triuidx[1]].view(-1)
            else:
                view = p.grad.view(-1)
            views.append(view)
    ...
    def _add_grad(self, step_size, update):
        ...
        for p in self._params:
            ...
            if isinstance(p, SOParameter):
                ...
                triuidx = torch.triu_indices(*p.grad.shape, offset=1).to(p.grad.device)
                numel = len(triuidx.T)
                theta = skew_symmetric(update[offset:offset + numel], triuidx, p.grad.shape)
                direction = expm(step_size * theta)
