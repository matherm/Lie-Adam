...
class SFA():
    ...
    def  __init__(self, n_components = 30, slow_components = 2, ica=False, shape=(3,32,32), BSZ=(16, 16), stride=4, mode="slow", temporal_decorr=False, act=lambda x : x):
    ...
    def init_model(self, n_components):
        self.model = SpatialICA(shape=self.shape, 
                           ...
                           padding=0, 
                           stride=self.stride, 
                           n_components=self.dim if n_components == -1 else n_components,
                           loss="negexp", 
    ...
    def fit(self, X, epochs = 15, bs=10000, logging=-1, lr=1e-2):
        ...
        if self.n_components == -1:
           n_positive_eigvals = max_entropy_gaussian(self.model.cov)
           self.n_components = n_positive_eigvals
           self.slow_components = n_positive_eigvals
           self.init_model(self.n_components)
           self.model.fit(X, epochs, X_val=X[:100], logging=logging, lr=lr, bs=bs)
           self.model.cpu()
        ...
        self.Var_diff           = S_change_score_diff.reshape(len(S), -1).var(0).sum()
    ...
    def hyperparameter_search(X, X_in, X_out, patch_size=[8, 16], n_components=[8, 16], stride=[2, 4], shape=(3,32,32), bs=10000, epochs=1, norm=[1], remove_components=None, logging=-1):
                    ...
                    if type(c) == str or c <= p*p*shape[0]:
                        try:
                            ...
                            for nor in norm:
                                ...
                                bpd_field    = bpd_pca_elbo_receptive(model.model, X, mean, std).mean()
                                ...
                                bookkeeping.append([p, s, model.n_components, nor, \
                                    k_min, k_max, k, bpd, bpd_field, \
                                        ...
                                        var_diff, KL, d_ruska, negH_diff, H_max, auc_lhd] + auc )
                        except NameError:
                            e = sys.exc_info()[0]
                            v = sys.exc_info()[1]
        ...
        return pd.DataFrame(bookkeeping, columns=["patch_size", "s", "n_components", "nor", \
                                    "k_min", "k_max", "k", "bpd", "bpd_field", \
                                        "H_receptive", "H_signal", "H_signal_white", "H_neighbor", "H_signal_sparse", "H_signal_gauss", "CE_gaussian", \
                                            "var_diff", "KL", "d_ruska", "negH_diff", "H_max", "lhd" ,"var", "sum", "mean"])       
